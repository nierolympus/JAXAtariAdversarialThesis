...
This variant adds a random adversarial mod action via GenericStateModWrapper/PongStateModWrapper.
"""
...
def make_train(config):
    ...
    adv_enabled = config.get("ADV_RANDOM", False)
    ...
    def batched_step(step_env, env_state, action, rng, step_idx):
        if adv_enabled:
            rng, rng_mask = jax.random.split(rng)
            mod_action = jax.random.randint(
                rng, shape=action.shape, minval=0, maxval=mod_action_space_n
            )
            if adv_mode == "prob":
                apply_mask = jax.random.bernoulli(rng_mask, adv_prob, shape=action.shape)
            ...
            return jax.vmap(step_env.step)(env_state, action, mod_action)
        return jax.vmap(step_env.step)(env_state, action)
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, step_idx):
                ...
                step_out = batched_step(env, env_state, new_action, rng_s, step_idx)
                new_obs, new_env_state, reward, new_termination, new_truncation, info = step_out
        ...
        def get_test_metrics(train_state, mod, rng):
            ...
            def _env_step(carry, step_idx):
                ...
                if mod:
                    step_out = batched_step(mod_env, env_state, action, rng_s, step_idx)
                else:
                    step_out = batched_step(env, env_state, action, rng_s, step_idx)
